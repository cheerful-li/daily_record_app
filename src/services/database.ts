...
export interface DailyRecordDB {
  habits: {
    key: number;
    value: Habit;
    indexes: { 'by-name': string; 'by-frequency': string; 'by-active': boolean };
  };
  checkIns: {
    key: number;
    value: CheckIn;
  ...
  tasks: {
    key: number;
    value: Task;
    indexes: { 'by-type': string; 'by-status': string; 'by-dueDate': Date; 'by-priority': string };
  };
  ideas: {
    key: number;
    value: Idea;
  ...
  relationships: {
    key: number;
    value: Relationship;
    indexes: { 'by-name': string; 'by-category': string; 'by-nextContact': Date };
  };
}
  ...
  const db = await openDB<DailyRecordDB>(DB_NAME, DB_VERSION, {
    upgrade(db, oldVersion, newVersion, transaction) {
      ...
      if (!db.objectStoreNames.contains('lifeMoments')) {
        const lifeMomentsStore = db.createObjectStore('lifeMoments', { keyPath: 'id', autoIncrement: true });
      ...
      if (!db.objectStoreNames.contains('relationships')) {
        const relationshipsStore = db.createObjectStore('relationships', { keyPath: 'id', autoIncrement: true });
...
export async function getAll<T extends keyof DailyRecordDB>(storeName: T): Promise<DailyRecordDB[T]['value'][]> {
  const db = await initDatabase();
  return db.getAll(storeName);
}

export async function getById<T extends keyof DailyRecordDB>(storeName: T, id: number): Promise<DailyRecordDB[T]['value'] | undefined> {
  const db = await initDatabase();
  return db.get(storeName, id);
}

export async function add<T extends keyof DailyRecordDB>(storeName: T, item: Omit<DailyRecordDB[T]['value'], 'id'>): Promise<number> {
  const db = await initDatabase();
  const now = new Date();
...
export async function update<T extends keyof DailyRecordDB>(storeName: T, id: number, item: Partial<DailyRecordDB[T]['value']>): Promise<number> {
  const db = await initDatabase();
...
export async function remove<T extends keyof DailyRecordDB>(storeName: T, id: number): Promise<void> {
  const db = await initDatabase();
  await db.delete(storeName, id);
}

// Helper function to query by index
export async function queryByIndex<T extends keyof DailyRecordDB, I extends keyof DailyRecordDB[T]['indexes']>(
  storeName: T,
  indexName: I,
